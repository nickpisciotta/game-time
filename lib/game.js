...
var allObstacles = [];
var allPlatforms = [];
var obstaclexValues = _.range(0, -500, -30);
var obstacleyValues = [210, 240, 270, 300, 330, 360, 390];
var platformyValues = [45, 75, 105, 145];
var platformxValues = _.range(0, -500, -30);
...
function Game(canvas, context) {
  ...
  this.platforms = makePlatform(canvas, context);
  this.level = 1;
...
Game.prototype.checkforObstacleCollisions = function(player) {
  var self = this;
...
Game.prototype.checkforPlatformCollisions = function(player) {
  var self = this;
  this.platforms.forEach(function(platform) {
    if (player.x < platform.x + platform.width && player.x +
      player.width > platform.x && player.y < platform.y +
      platform.height && player.height + player.y> platform.y) {
...
Game.prototype.moveOnPlatform = function() {
  this.player.moveRight();
};

Game.prototype.checkForCollision = function(player) {
  if (this.player.y > 180) {
    this.checkforObstacleCollisions(player);
  } else if (this.player.y < 160 && this.player.y > 10)
   {
    this.checkforPlatformCollisions(player);
  };
};
...
function makeObstacle(canvas, context) {
  _(15).times(function() {
    var newObstacle = new Obstacle({y: _.sample(obstacleyValues), x: _.sample(obstaclexValues), width: 30,
...
function makePlatform(canvas, context) {
  _(15).times(function() {
    var newPlatform = new Platform({y: _.sample(platformyValues), x: _.sample(platformxValues), width: 30,
      height: 20, canvas: canvas, context: context });
